...
jQuery(function($) {
  ...
  $('#promo_checkout a[href="#cc"]').click(function(e) {
    e.preventDefault();
    	...
    	var cartUrl = 'https://'+storedomain+'/cart?cart=view';
		var fccSession = fcc.session_get();
		var quantity = promoSelectedPackage;
		$.ajax({
		   type: 'POST',
		   dataType: 'jsonp',
		   url: location.protocol+'//'+window.location.hostname+'/sample-cart/',
		   data: {
			   storedomain: storedomain,
				fccSession: fccSession,
				quantity:	quantity							   
		   },
		   success: function(r) {

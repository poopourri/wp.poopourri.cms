...
<div id="foxyshop_container" style="padding-top:50px;">
...
if (isset($_POST['transaction_id_search'])) {
	...
	if ($xml->result == "ERROR") {
		...
		foreach($xml->transaction as $transaction) {
			...
			if($tracking_info=='Call'){
				echo '<p><b>Tracking number unavailable.</b> Please allow 2 business days from when you ordered for your tracking number to update. Please <a href="/customer-services/contact-us/">contact us</a> if you think your tracking number should be available.</p>';
			}else{
...
if ($wp_user_id == 0) {
	...
	echo '</form><br/><br/>' . "\n";
	echo '<p>Here is an example of an order email...</p><p><img src="/wp-content/uploads/2013/08/EmailOrderExample.jpg"/></p>';
...
//User Is Logged In
	...
	if ($xml->result == "ERROR") {
		...
		if ($pagination_start > 1 || $filtered_total > $pagination_end) {
			...
			if ($pagination_start > 1) echo '<a href="/myorders/' . $querystring . '&amp;pagination_start=' . ($pagination_start - $p - 1) . '">&laquo; Previous</a>';
			if ($pagination_end < $filtered_total) {
				if ($pagination_start > 1) echo ' | ';
				echo '<a href="/myorders/' . $querystring . '&amp;pagination_start=' . $pagination_end . '">Next &raquo;</a>';
			}
			echo '</div>';
		}
	}
